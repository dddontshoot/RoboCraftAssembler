#RobocraftAssembler0.3.3.py
# this version has been broken up into modules.
# The default cube causes an issue when you run from the command line.
...
def makeitso(cubeDataHex,colourDataHex,cubeCount,cubedatabase):
    ...
    for x in range(0,cubeCount):
        ...
        for filename in objectlist: # filename is a string. it is the name of the object we're importing from the library.
            ...
            # duplicating works on selected objects, so if I have to make sure I unselect everything first.
            unselecteverything()
...
def main(botfile):
    ...
    unselecteverything() # lets unselect everything so that we don't get any pre-existing cubes duplicated
    ...
    #########################################################
    # open the .bot file and look for cudeData and colourData
    #########################################################
    
    
    print("\nNow building",botfile,"...")
    
    cubeDataHex,colourDataHex,cubeCount=getbotfile(botfile)
    ...
    cubedatabase=getdatabase("cubes.csv")
